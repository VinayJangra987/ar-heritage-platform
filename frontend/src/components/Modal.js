...
//   const [rvImages, setRvImages]     = useState([]);   // NEW: File objects
//   const [rvPreviews, setRvPreviews] = useState([]);   // NEW: preview URLs
...
//   // Stop any audio narration when modal unmounts
//   useEffect(() => {
...
//   // NEW: image selection for review form
//   const handleRvImageSelect = (e) => {
//     const files = Array.from(e.target.files).slice(0, 5 - rvImages.length);
//     if (!files.length) return;
//     setRvImages((prev) => [...prev, ...files]);
//     setRvPreviews((prev) => [...prev, ...files.map((f) => URL.createObjectURL(f))]);
//   };

//   const removeRvImage = (idx) => {
//     setRvImages((prev) => prev.filter((_, i) => i !== idx));
//     setRvPreviews((prev) => prev.filter((_, i) => i !== idx));
//   };
...
//   // 🔊 Audio narration — reads the site's description aloud
//   const handleNarrate = () => {
//     if (speaking) {
...
//       return;
//     }
//     const text = `${site.name}. ${site.description || site.shortDesc || ""}. ${site.significance || ""}`;
...
//     const shareData = {
//       title: site.name,
//       text: `Check out ${site.name} — ${site.district}, ${site.state} on Bharatiya Dharohar!`,
...
//         /* NEW: photo upload in Modal reviews tab */
//         .rv-img-upload-label {
...
//               title={speaking ? "Stop narration" : "Listen to description"}
//             >
//               {speaking ? "⏸️" : "🔊"}
//             </button>
...
//                     </p>
//                   )}
//                   <a href={`https://www.google.com/maps?q=${site.name} ${site.state}`}
...
//                             <button className="rv-img-remove" onClick={() => removeRvImage(i)}>✕</button>
//                           </div>
//                         ))}
//                       </div>
//                     )}
...
//                         📷 Add Photos ({rvImages.length}/5)
//                         <input
//                           type="file"
//                           accept="image/*"
//                           multiple
//                           onChange={handleRvImageSelect}
//                           style={{ display: "none" }}
//                         />
//                       </label>
//                     )}
...
export default function Modal({ site, onClose, recommendations, onNavigate, onARClick, isFav, onToggleFav }) {
  ...
  const [comment, setComment]     = useState("");
  const [title, setTitle]         = useState("");
  const [rvImages, setRvImages]     = useState([]);
...
const siteId = /^[a-f\d]{24}$/i.test(site?._id) ? site._id : null;
  ...
  const handleLike = async (reviewId) => {
    ...
    try {
      const data = await reviewsAPI.likeReview(reviewId);
      setReviews((prev) =>
        prev.map((r) =>
          r._id === reviewId
            ? {
      ...
      // silently fail — like is a low-stakes action
    }
    setLikingId(null);
  };
  ...
  return (
    <>
      <style>{`
        ...
        /* Like / delete actions on a review card */
        .rv-card-actions { display: flex; align-items: center; gap: 0.75rem; margin-top: 0.6rem; }
        .rv-like-btn {
          background: rgba(255,255,255,0.04); border: 1px solid rgba(201,168,76,0.15);
          border-radius: 20px; padding: 4px 12px; cursor: pointer;
          font-family: 'Space Mono', monospace; font-size: 0.65rem;
          color: rgba(242,232,208,0.6); transition: all 0.2s;
        }
        .rv-like-btn:hover { border-color: rgba(201,168,76,0.4); }
        .rv-like-btn.liked { color: #E24B4A; border-color: rgba(224,75,74,0.3); }
        .rv-like-btn:disabled { opacity: 0.5; cursor: not-allowed; }
        .rv-delete-btn {
          background: rgba(224,75,74,0.08); border: 1px solid rgba(224,75,74,0.25);
          border-radius: 20px; padding: 4px 12px; cursor: pointer;
          font-family: 'Space Mono', monospace; font-size: 0.62rem;
          color: rgba(224,75,74,0.85); transition: all 0.2s;
        }
        .rv-delete-btn:hover { background: rgba(224,75,74,0.15); }
        .rv-delete-btn:disabled { opacity: 0.5; cursor: not-allowed; }
              ...
              <>
                ...
                {rvLoading ? (
                  ...
                  reviews.map((r) => {
                    ...
                    const isOwner = user && r.user?._id === user._id;
                    return (
                      <div key={r._id} className="rv-card">
                        <div className="rv-card-top">
                          <div className="rv-card-name">{r.user?.name || "Anonymous"}</div>
                        ...
                        <div className="rv-card-actions">
                          <button
                            className={`rv-like-btn ${isLiked ? "liked" : ""}`}
                            onClick={() => handleLike(r._id)}
                            disabled={likingId === r._id}
                          >
                            {isLiked ? "❤️" : "🤍"} {r.likes?.length || 0}
                          </button>
                          {isOwner && (
                            <button
