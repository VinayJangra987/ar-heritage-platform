...
export default function ReviewSystem({ site, onClose }) {
  ...
  const [previews,   setPreviews]   = useState([]);      // NEW: preview URLs
  const [submitting, setSubmitting] = useState(false);
  const [error,      setError]      = useState("");
  const [success,    setSuccess]    = useState("");
  ...
  const handleSubmit = async () => {
    ...
    if (!siteId)  { setError("This site does not support reviews yet."); return; }

    setSubmitting(true);
    setError("");

    try {
      const data = await reviewsAPI.addReview(siteId, { rating, title, comment, images });
      if (data.review) {
        setReviews((prev) => [data.review, ...prev]);
  ...
  return (
    <>
      <style>{`
        .rv-overlay { position: fixed; inset: 0; z-index: 9999; background: rgba(0,0,0,0.8); display: flex; align-items: center; justify-content: center; padding: 1rem; }
        .rv-box { background: #0D1B2A; border: 1px solid rgba(201,168,76,0.3); border-radius: 16px; width: 100%; max-width: 560px; max-height: 85vh; overflow-y: auto; padding: 2rem; position: relative; }
        .rv-title { font-family: 'Cormorant Garamond', serif; font-size: 1.6rem; color: #C9A84C; margin-bottom: 0.3rem; }
        .rv-site-name { font-family: 'Space Mono', monospace; font-size: 0.55rem; letter-spacing: 0.2em; color: rgba(242,232,208,0.4); text-transform: uppercase; margin-bottom: 1.5rem; }
        .rv-close { position: absolute; top: 1rem; right: 1rem; background: none; border: none; color: rgba(242,232,208,0.4); font-size: 1.2rem; cursor: pointer; }
        .rv-no-backend { background: rgba(201,168,76,0.05); border: 1px dashed rgba(201,168,76,0.2); border-radius: 10px; padding: 1.2rem; margin-bottom: 1.5rem; text-align: center; }
        .rv-no-backend-icon { font-size: 1.8rem; margin-bottom: 0.5rem; }
        .rv-no-backend-text { font-family: 'Poppins', sans-serif; font-size: 0.78rem; color: rgba(242,232,208,0.4); line-height: 1.6; }
        .rv-no-backend-text strong { color: rgba(201,168,76,0.7); display: block; margin-bottom: 3px; }
        ...
        .rv-form-title { font-family: 'Space Mono', monospace; font-size: 0.58rem; letter-spacing: 0.15em; color: #C9A84C; text-transform: uppercase; margin-bottom: 1rem; display: flex; align-items: center; gap: 0.5rem; }
        .rv-stars { display: flex; gap: 0.3rem; margin-bottom: 1rem; }
        .rv-star { font-size: 1.4rem; cursor: pointer; transition: transform 0.15s; }
        .rv-star:hover { transform: scale(1.2); }
        ...
        .rv-img-upload-label {
          ...
          padding: 0.5rem 1rem; border: 1px dashed rgba(201,168,76,0.3);
          border-radius: 8px; cursor: pointer;
          font-family: 'Space Mono', monospace; font-size: 0.6rem;
          color: rgba(201,168,76,0.8); letter-spacing: 0.08em;
          margin-bottom: 0.75rem; transition: all 0.2s;
        }
        .rv-img-upload-label:hover { border-color: #C9A84C; background: rgba(201,168,76,0.05); }
        .rv-img-previews { display: flex; gap: 0.5rem; flex-wrap: wrap; margin-bottom: 0.75rem; }
        .rv-img-preview-wrap { position: relative; width: 64px; height: 64px; }
        .rv-img-preview { width: 64px; height: 64px; object-fit: cover; border-radius: 8px; border: 1px solid rgba(201,168,76,0.2); }
        .rv-img-remove {
        ...
        .rv-card-img-strip { display: flex; gap: 0.4rem; margin-top: 0.5rem; flex-wrap: wrap; }
        .rv-card-img { width: 56px; height: 56px; object-fit: cover; border-radius: 6px; cursor: pointer; }

        .rv-submit { width: 100%; padding: 0.8rem; background: linear-gradient(135deg, #C9A84C, #E8C96A); color: #0D1B2A; border: none; border-radius: 8px; font-family: 'Space Mono', monospace; font-weight: 700; font-size: 0.62rem; letter-spacing: 0.12em; text-transform: uppercase; cursor: pointer; transition: opacity 0.2s; }
        .rv-submit:disabled { opacity: 0.6; cursor: not-allowed; }
        .rv-error   { color: #E24B4A; font-size: 0.75rem; margin-bottom: 0.5rem; }
        .rv-success { color: #4AC878; font-size: 0.75rem; margin-bottom: 0.5rem; }
        ...
        .rv-card-date { font-size: 0.65rem; color: rgba(242,232,208,0.3); }
        .rv-card-stars { margin-bottom: 0.4rem; font-size: 0.9rem; }
        .rv-card-title { font-size: 0.85rem; font-weight: 600; color: #F2E8D0; margin-bottom: 0.3rem; }
        .rv-card-comment { font-size: 0.8rem; color: rgba(242,232,208,0.6); line-height: 1.6; }
        .rv-empty { text-align: center; color: rgba(242,232,208,0.3); font-size: 0.8rem; padding: 1.5rem 0; }
        .rv-login-msg { text-align: center; color: rgba(242,232,208,0.4); font-size: 0.78rem; padding: 0.75rem; border: 1px dashed rgba(201,168,76,0.2); border-radius: 8px; margin-bottom: 1.5rem; }
      `}</style>
        ...
        <div className="rv-box">
          ...
          {!siteId ? (
            <div className="rv-no-backend">
              ...
              <div className="rv-no-backend-text">
                <strong>Backend connection needed</strong>
                This site is using local data. Reviews will work once the backend is connected to MongoDB.
              </div>
            </div>
          ) : user ? (
            <div className="rv-form">
              ...
              {/* NEW: image upload */}
              {previews.length > 0 && (
                <div className="rv-img-previews">
                  {previews.map((src, i) => (
                    <div key={i} className="rv-img-preview-wrap">
                      <img src={src} alt="" className="rv-img-preview" />
                      <button className="rv-img-remove" onClick={() => removeImage(i)}>✕</button>
                    </div>
                  ))}
                </div>
              )}
              {images.length < 5 && (
                <label className="rv-img-upload-label">
          ...
          {siteId && (
            <>
              ...
              {loading ? (
                ...
                <div className="rv-empty">No reviews yet — be the first to write one! ✍️</div>
              ) : (
                reviews.map((r) => (
                  <div key={r._id} className="rv-card">
                    <div className="rv-card-top">
                    ...
                    {/* NEW: photo strip */}
                    {r.images?.length > 0 && (
                      <div className="rv-card-img-strip">
                        {r.images.map((img, i) => (
                          <img
                            key={i}
                            src={img.url}
