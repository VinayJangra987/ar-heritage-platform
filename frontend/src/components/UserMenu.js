  ...
  return (
    <div className="um-wrap" ref={ref}>
      <button className="um-avatar-btn" onClick={() => setOpen(!open)}>
        ...
        <span className="um-name">{user.name.split(' ')[0]}</span>
        {streakCurrent > 0 && (
          <span className="um-streak-pill" title={`${streakCurrent}-day streak`}>
            🔥 {streakCurrent}
          </span>
        )}
        <span className="um-chevron">{open ? '▲' : '▼'}</span>
        ...
        <div className="um-dropdown">
          ...
          <div className="um-stats-row">
            <div className="um-stat-box">
              <div className="um-stat-num">🔥 {streakCurrent}</div>
              <div className="um-stat-label">Day Streak</div>
            </div>
            <div className="um-stat-box">
              <div className="um-stat-num">🏅 {badgeCount}</div>
              <div className="um-stat-label">Badges</div>
            </div>
          </div>
          ...
          {badgeCount > 0 && (
            <div className="um-badges-strip">
              {user.badges.map((b) => (
                <span key={b.id} className="um-badge-chip" title={b.id}>
                  {b.id.includes("streak") ? "🔥" : b.id.includes("collector") ? "❤️" : b.id.includes("review") ? "✍️" : "🏅"}
                </span>
              ))}
            </div>
          )}
