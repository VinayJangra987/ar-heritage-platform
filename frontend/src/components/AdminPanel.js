...
//     addedBy: "admin@dharohar.com", addedAt: "2024-01-01", status: "published",
//   },
//   {
//     id: "taj-001",
//     name: "Taj Mahal",
...
// };
// const saveMonuments = (list) => localStorage.setItem("dharohar_monuments", JSON.stringify(list));

// const emptyForm = {
//   name:"", state:"", address:"", era:"", type:"architectural",
...
//       const res = await fetch(`${API_BASE}/heritage?limit=100`, {
//         headers: { Authorization: `Bearer ${token}` },
//       });
//       const data = await res.json();
//       if (data.sites?.length > 0) {
//         setMonuments(data.sites);
//         saveMonuments(data.sites);
//       }
//     } catch (err) {
...
//       if (!response.ok) throw new Error(data.message || "Backend save failed");
   
//       window.dispatchEvent(new Event("monumentAdded"));
...
//   const updateToBackend = async (id, monumentData) => {
//     try {
//       const token = localStorage.getItem("token");
//       const response = await fetch(`${API_BASE}/heritage/${id}`, {
//         method: "PATCH",
//         headers: {
//           "Content-Type": "application/json",
//           Authorization: `Bearer ${token}`,
...
//       });
//       const data = await response.json();
//       if (!response.ok) throw new Error(data.message || "Backend update failed");
//       window.dispatchEvent(new Event("monumentAdded"));
//       return { success: true, data };
//     } catch (err) {
//       return { success: false, error: err.message };
...
//       });
//       const data = await response.json();
//       if (!response.ok) throw new Error(data.message || "Backend delete failed");
//       window.dispatchEvent(new Event("monumentAdded"));
//       return { success: true };
//     } catch (err) {
//       return { success: false, error: err.message };
...
//       if (!result.success) {
//         setApiError(`Backend update failed: ${result.error}. Local mein save kar raha hoon.`);
//       }
//       const updated = monuments.map(m =>
//         (m._id || m.id) === backendId ? { ...m, ...monumentData, image: form.image } : m 
//       );
//       setMonuments(updated);
//       saveMonuments(updated);
//     } else {
//       // ── Add: Backend pe save karo, phir localStorage ──
//       const result = await saveToBackend(monumentData);
//       let newM;
//       if (result.success) {
//         newM = result.data.site || result.data;
//       } else {
//         setApiError(`Backend save failed: ${result.error}. Save Locally.`);
//         newM = { ...monumentData, id: `monument-${Date.now()}` };
//       }
...
//     if (!result.success) {
//       setApiError(`Backend delete failed: ${result.error}. Delete From the local.`);
//     }
//     const updated = monuments.filter(m => (m._id || m.id) !== id);
//     setMonuments(updated);  
//     saveMonuments(updated);
...
//   // list response mein description missing hai, to full record fetch karo
//   let full = m;
//   const id = m._id || m.id;
...
//       });
//       const data = await res.json();
//       if (data.site) full = data.site;
//     } catch (err) {
//       console.error("Failed to fetch full site details:", err);
//     }
//   }
...
//     state: full.state || "",
//     address: full.address || `${full.district || ""}, ${full.state || ""}`.replace(/^, /, ""),
//     era: full.era || "",
//     type: (full.type || "architectural").toLowerCase(),
//     description: full.description || "",
//     facts: (full.highlights || full.facts || []).join("\n"),
//     image: full.image || full.thumbnail || full.images?.[0] || "",
//     sketchfabId: full.sketchfabId || "",
//     lat: full.lat || full.location?.coordinates?.[1] || "",
//     lng: full.lng || full.location?.coordinates?.[0] || "",
...
//   const matchSearch = !q || m.name.toLowerCase().includes(q) || m.district?.toLowerCase().includes(q) || m.state?.toLowerCase().includes(q);
//   const matchType   = filterType === "all" || m.type?.toLowerCase() === filterType.toLowerCase(); 
//   const matchStatus = filterStatus === "all" || (m.status || "published") === filterStatus;
...
//         .adm-overlay{position:fixed;inset:0;z-index:10002;background:#06101A;display:flex;flex-direction:column;font-family:'Poppins',sans-serif;overflow:hidden;}
//         .adm-topbar{display:flex;align-items:center;justify-content:space-between;padding:0.9rem 1.5rem;background:rgba(8,18,30,0.95);border-bottom:1px solid rgba(201,168,76,0.12);flex-shrink:0;}
//         .adm-topbar-left{display:flex;align-items:center;gap:1rem;}
//         .adm-brand{font-family:'Cormorant Garamond',serif;font-size:1.1rem;font-weight:700;color:#F2E8D0;display:flex;align-items:center;gap:0.5rem;}
//         .adm-brand-badge{font-family:'Space Mono',monospace;font-size:0.45rem;letter-spacing:0.15em;text-transform:uppercase;background:rgba(201,168,76,0.15);color:#C9A84C;border:1px solid rgba(201,168,76,0.25);padding:3px 8px;border-radius:20px;}
//         .adm-topbar-right{display:flex;align-items:center;gap:0.75rem;}
//         .adm-user-pill{display:flex;align-items:center;gap:0.5rem;padding:5px 12px;background:rgba(242,232,208,0.05);border:1px solid rgba(242,232,208,0.1);border-radius:50px;font-family:'Space Mono',monospace;font-size:0.52rem;color:rgba(242,232,208,0.5);}
//         .adm-user-dot{width:6px;height:6px;border-radius:50%;background:#4AC878;}
//         .adm-topbar-btn{padding:6px 14px;border-radius:8px;font-family:'Space Mono',monospace;font-size:0.52rem;letter-spacing:0.1em;text-transform:uppercase;cursor:pointer;transition:all 0.2s;border:none;}
...
//         .adm-nav-item{display:flex;align-items:center;gap:0.6rem;padding:0.6rem 0.8rem;border-radius:10px;font-size:0.78rem;color:rgba(242,232,208,0.4);cursor:pointer;transition:all 0.2s;background:transparent;border:none;text-align:left;width:100%;}
...
//         .adm-api-error{background:rgba(220,80,80,0.08);border:1px solid rgba(220,80,80,0.2);border-radius:8px;padding:0.7rem 1rem;font-size:0.75rem;color:rgba(255,120,120,0.9);margin-bottom:1rem;display:flex;align-items:center;gap:0.5rem;}
//         .adm-stats-row{display:grid;grid-template-columns:repeat(4,1fr);gap:1rem;margin-bottom:1.5rem;}
//         .adm-stat-card{background:rgba(8,18,30,0.8);border:1px solid rgba(201,168,76,0.1);border-radius:14px;padding:1.2rem;}
...
//         .adm-section-title{font-family:'Cormorant Garamond',serif;font-size:1.3rem;font-weight:700;color:#F2E8D0;}
//         .adm-filters{display:flex;gap:0.75rem;margin-bottom:1rem;flex-wrap:wrap;}
//         .adm-search{flex:1;min-width:200px;padding:0.6rem 1rem;background:rgba(242,232,208,0.04);border:1px solid rgba(242,232,208,0.1);border-radius:8px;color:#F2E8D0;font-family:'Poppins',sans-serif;font-size:0.82rem;outline:none;}
//         .adm-search:focus{border-color:rgba(201,168,76,0.3);}
//         .adm-select{padding:0.6rem 0.9rem;background:rgba(242,232,208,0.04);border:1px solid rgba(242,232,208,0.1);border-radius:8px;color:rgba(242,232,208,0.7);font-family:'Space Mono',monospace;font-size:0.52rem;letter-spacing:0.08em;outline:none;cursor:pointer;}
...
//         .adm-monument-card{background:rgba(8,18,30,0.8);border:1px solid rgba(242,232,208,0.07);border-radius:14px;overflow:hidden;transition:all 0.2s;}
...
//         .adm-monument-type{font-family:'Space Mono',monospace;font-size:0.46rem;letter-spacing:0.15em;text-transform:uppercase;padding:3px 8px;border-radius:20px;display:inline-block;margin-bottom:0.5rem;}
...
//         .adm-monument-status{font-family:'Space Mono',monospace;font-size:0.45rem;letter-spacing:0.12em;text-transform:uppercase;padding:3px 8px;border-radius:20px;}
...
//         .adm-card-btn{padding:5px 10px;border-radius:6px;font-family:'Space Mono',monospace;font-size:0.48rem;letter-spacing:0.08em;cursor:pointer;border:none;transition:all 0.2s;}
...
//         .adm-form-label{font-family:'Space Mono',monospace;font-size:0.48rem;letter-spacing:0.15em;text-transform:uppercase;color:rgba(242,232,208,0.4);}
//         .adm-form-input,.adm-form-textarea,.adm-form-select{padding:0.7rem 0.9rem;background:rgba(242,232,208,0.04);border:1px solid rgba(242,232,208,0.1);border-radius:10px;color:#F2E8D0;font-family:'Poppins',sans-serif;font-size:0.85rem;outline:none;transition:all 0.2s;}
...
//         .adm-img-placeholder{width:100%;height:100px;border-radius:10px;margin-top:0.5rem;border:1px dashed rgba(242,232,208,0.15);display:flex;align-items:center;justify-content:center;font-size:0.7rem;color:rgba(242,232,208,0.2);}
//         .adm-form-divider{border:none;border-top:1px solid rgba(242,232,208,0.07);margin:1.2rem 0;}
//         .adm-form-section-label{font-family:'Space Mono',monospace;font-size:0.5rem;letter-spacing:0.2em;text-transform:uppercase;color:rgba(201,168,76,0.5);margin-bottom:0.75rem;display:flex;align-items:center;gap:0.5rem;}
...
//         .adm-btn{padding:0.8rem 1.8rem;border-radius:10px;font-family:'Space Mono',monospace;font-size:0.58rem;letter-spacing:0.12em;text-transform:uppercase;font-weight:700;cursor:pointer;transition:all 0.25s;border:none;}
...
//         .adm-save-msg{position:fixed;bottom:2rem;right:2rem;z-index:99999;background:rgba(74,200,120,0.15);border:1px solid rgba(74,200,120,0.3);border-radius:10px;padding:0.75rem 1.5rem;font-family:'Space Mono',monospace;font-size:0.6rem;letter-spacing:0.1em;color:rgba(74,200,120,0.9);display:flex;align-items:center;gap:0.5rem;animation:adm-slideup 0.3s ease;}
//         @keyframes adm-slideup{from{opacity:0;transform:translateY(10px)}to{opacity:1;transform:translateY(0)}}
//         .adm-confirm-overlay{position:fixed;inset:0;z-index:10010;background:rgba(0,0,0,0.7);backdrop-filter:blur(4px);display:flex;align-items:center;justify-content:center;}
//         .adm-confirm-box{background:#0D1B2A;border:1px solid rgba(220,80,80,0.3);border-radius:16px;padding:2rem;max-width:360px;width:90%;text-align:center;}
...
//         .adm-empty{display:flex;flex-direction:column;align-items:center;justify-content:center;padding:4rem;text-align:center;gap:1rem;}
//         .adm-empty-icon{font-size:3rem;opacity:0.3;}
//         .adm-empty-text{font-size:0.9rem;color:rgba(242,232,208,0.3);}
//         @media(max-width:768px){.adm-stats-row{grid-template-columns:1fr 1fr;}.adm-form-grid{grid-template-columns:1fr;}.adm-sidebar{display:none;}}
//       `}</style>
...
//               🏛 Dharohar <span className="adm-brand-badge">Admin</span>
//             </div>
//           </div>
...
//               disabled={apiLoading}
//               onClick={() => { setForm(emptyForm); setEditTarget(null); setFormErrors({}); setView("add"); }}
//             >
//               + Add Monument
//             </button>
...
//                 onClick={() => {
//                   if (item.id === "add") { setForm(emptyForm); setEditTarget(null); setFormErrors({}); }
//                   setView(item.id);
//                 }}
//               >
...
//                 <button
//                   onClick={() => setApiError("")}
//                   style={{ marginLeft:"auto", background:"none", border:"none", color:"rgba(255,120,120,0.7)", cursor:"pointer" }}
//                 >✕</button>
//               </div>
//             )}
...
//                      { label:"Published",        num:monuments.filter(m=>(m.status || "published")==="published").length,       sub:"Live on app"   }, 
...
//                     {filtered.map(m => (
//                       <div className="adm-monument-card" key={m._id || m.id}>
//                         <img
//                           className="adm-monument-img"
//                           src={m.image || m.images?.[0] || "https://images.unsplash.com/photo-1564507592333-c60657eea523?w=400"}
//                           alt={m.name}
...
//                           <div className="adm-monument-loc">
//                              📍 {typeof m.location === "string" ? m.location : (m.address || `${m.district || ""}, ${m.state || ""}`)} · {m.era}
//                            </div>
...
//                   </div>
//                   <div className="adm-form-group">
//                     <label className="adm-form-label">Full Address *</label>
//                     <input className={`adm-form-input ${formErrors.address?"err":""}`} placeholder="e.g. New Delhi, Delhi" value={form.address} onChange={e=>setField("address",e.target.value)} />
//                     {formErrors.address && <span className="adm-form-err">{formErrors.address}</span>}
//                   </div>
//                   <div className="adm-form-group">
...
//                     <input className={`adm-form-input ${formErrors.image?"err":""}`} placeholder="https://images.unsplash.com/..." value={form.image} onChange={e=>setField("image",e.target.value)} />
...
//                   </div>
//                   <div className="adm-form-group adm-form-full">
//                     <label className="adm-form-label">Sketchfab Model ID</label>
...
//                   <button className="adm-btn primary" onClick={handleSave} disabled={apiLoading}>
//                     {apiLoading ? "⏳ Saving..." : editTarget ? "💾 Save Changes" : "✅ Add Monument"}
//                   </button>
