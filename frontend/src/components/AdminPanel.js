...
export default function AdminPanel({ onClose, onMonumentsUpdate, adminUser }) {
  ...
  const updateToBackend = async (id, monumentData) => {
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(`${API_BASE}/api/heritage/${id}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
  ...
  const handleSave = async () => {
    ...
    if (editTarget) {
      ...
      const updated = monuments.map(m =>
        (m._id || m.id) === backendId ? { ...m, ...monumentData } : m
      );
      ...
      if (result.success) {
        ...
        setApiError(`Backend save failed: ${result.error}. Local mein save kar raha hoon.`);
  ...
  const handleDelete = async (id) => {
    ...
    if (!result.success) {
      setApiError(`Backend delete failed: ${result.error}. Local se delete kar raha hoon.`);
    }
