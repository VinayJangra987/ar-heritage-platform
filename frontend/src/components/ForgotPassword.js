...
//         {step === 'otp' && (
//           <>
//             <h2 style={titleStyle}>Verify the OTP</h2>
//             <p style={descStyle}>
...
if (typeof forgotPassword !== 'function') {
  setError('Forgot password service is not configured correctly');
  console.error('forgotPassword is not a function:', forgotPassword);
  return;
}
...
if (typeof verifyResetOtp !== 'function') {
  setError('OTP verification service is not configured correctly');
  console.error('verifyResetOtp is not a function:', verifyResetOtp);
  return;
}
...
if (!resetToken) {
  setError('Reset token is missing. Please verify the OTP again.');
  return;
}

if (typeof resetPassword !== 'function') {
  setError('Password reset service is not configured correctly');
...
try {
  ...
  setError(err?.message || 'Something went wrong while resetting the password');
} finally {
  setLoading(false);
}
};
